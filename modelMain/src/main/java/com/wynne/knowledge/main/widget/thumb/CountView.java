...
public class CountView extends View {
    ...
    public CountView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mTextSize = attrsArray.getDimension(R.styleable.CountView_count_size, StringUtils.dip2px(context, 15f));
        attrsArray.recycle();
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //变化前部分
        mTextPaint.setColor((Integer) StringUtils.evaluate(mFraction, mEndTextColor, mTextColor));
        canvas.drawText(String.valueOf(mTexts[1]), mTextPoints[1].x, mTextPoints[1].y, mTextPaint);

        //变化后部分
        mTextPaint.setColor((Integer) StringUtils.evaluate(mFraction, mTextColor, mEndTextColor));
